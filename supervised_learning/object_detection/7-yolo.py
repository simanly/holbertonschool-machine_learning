...
class Yolo:
    ...
    def predict(self, folder_path):
        """
        Displays all images with bounding boxes and returns predictions and image paths
        """
        ...
        for i in range(len(images)):
            outputs_i = [output[i] for output in outputs]

            boxes, box_confidences, box_class_probs = self.process_outputs(
                outputs_i, image_shapes[i]
            )

            filt_boxes, filt_classes, filt_scores = self.filter_boxes(
                boxes, box_confidences, box_class_probs
            )

            boxes, box_classes, box_scores = self.non_max_suppression(
                filt_boxes, filt_classes, filt_scores
            )
