...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Return the midpoints between consecutive feature values."""
        values = np.unique(
            (self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Return the best threshold and its Gini average for a feature."""
        ...
        # (n, c) : individual i is of class k
        class_membership = np.equal(
            target_values[:, None], classes[None, :])
        # (n, t) : feature value of i is above threshold j
        above = feature_values[:, None] > thresholds[None, :]

        # (n, t, c) booleans for left (above) and right (not above)
        left_f = above[:, :, None] & class_membership[:, None, :]
        right_f = (~above)[:, :, None] & class_membership[:, None, :]

        # (t, c) class counts per threshold
        ...
        gini_left = 1 - np.sum(
            (left_counts / left_total[:, None]) ** 2, axis=1)
        gini_right = 1 - np.sum(
            (right_counts / right_total[:, None]) ** 2, axis=1)

        gini_avg = (left_total * gini_left +
                    right_total * gini_right) / n

        min_index = np.argmin(gini_avg)
        return thresholds[min_index], gini_avg[min_index]

    def Gini_split_criterion(self, node):
        """Return the feature and threshold minimizing the Gini average."""
